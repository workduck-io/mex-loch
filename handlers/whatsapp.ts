...
app.get('/whatsapp', (req, res) => {
   ...
   **/
  const verify_token = 'qwertyasdfg'
  ...
  if (req.body.object) {
    if (
      ...
      try {
        ...
        //check if the unique message id in messageIds array
        const hasDuplicates = (arr) => arr.length !== new Set(arr).size
        if (!hasDuplicates(messageIds)) {
          const replyMessage = await messageService.handleMessage({
            message: msg_body,
            serviceId: from,
            sourceUrl: `https://w.me/Mex-Offical-Bot/${req.body.entry[0].changes[0].value.messages[0].id}`,
